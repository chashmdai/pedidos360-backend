package cl.duoc.pedidos360.bff.infrastructure.security;
...
public class SecurityConfiguration {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http,
            @Value("${pedidos.security.cors-enabled:true}") boolean corsEnabled) throws Exception {
        ...
        return http.csrf(csrf -> csrf.disable())
            .cors(cors -> { if (!corsEnabled) cors.disable(); })
            .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/actuator/health").permitAll()
                .requestMatchers(org.springframework.http.HttpMethod.GET, "/api/v1/me").hasAnyAuthority("SCOPE_Catalog.Read", "SCOPE_Orders.Read", "SCOPE_Orders.Create")
